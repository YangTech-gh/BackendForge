...
dbt test
dbt docs generate
`', '[{"id":"tc-1","description":"Models follow staging -> intermediate -> mart pattern","order":1,"required":true},{"id":"tc-2","description":"Tests catch data quality issues on every run","order":2,"required":true},{"id":"tc-3","description":"Incremental model processes only new rows","order":3,"required":true}]', 2, '{"prerequisiteLabId": "lab-debezium-cdc", "stage": "Building", "estimatedHours": 5, "learningObjective": "Build data transformation pipelines with dbt for analytics", "buildsToward": "Real-Time Data Platform"}', '["dbt models are SQL SELECT statements - keep them simple","Tests run on every dbt run - make them fast and meaningful","Incremental models are essential for large event tables","Use dbt variables for environment-specific configurations","Ref statements create dependency graphs - verify with dbt docs"]', '["Staging models clean and rename, intermediate transform, marts serve","Source freshness checks prevent stale data from propagating","dbt docs are the single source of truth for data documentation","The on_schema_change parameter in incremental models prevents silent data loss","Mart models should be the only ones exposed to BI tools"]', '["Use dbt packages for common transformations","Implement snapshot tables for slowly changing dimensions","Add exposure definitions for downstream BI tools","Connect this pipeline to the CDC events from lab-debezium-cdc for a complete data platform","Write custom generic tests for business-rule validation"]') ON CONFLICT (id) DO NOTHING;