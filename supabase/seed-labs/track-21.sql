...
`', '[{"id":"tc-1","description":"Consumer contract accurately describes expected API behavior","order":1,"required":true},{"id":"tc-2","description":"Provider passes all contract interactions","order":2,"required":true},{"id":"tc-3","description":"Breaking API change fails the provider contract test","order":3,"required":true}]', 2, '{"prerequisiteLabId": "lab-testing-patterns", "stage": "Building", "estimatedHours": 6, "learningObjective": "Implement consumer-driven contract testing for microservices", "buildsToward": "Production Quality Engineering"}', '["Contracts are living documentation - they describe actual API behavior","Pact Broker enables distributed teams to coordinate API changes","Consumer-driven contracts shift API design to the consumer side","Use matchers in Pact to avoid brittle exact-value matching","Tag consumer versions with release tags for traceability"]', '["Contract tests are not integration tests - they verify the API shape","Pact uses real HTTP calls in tests, not mocks","Tag consumer versions with release tags for traceability","Pact verifications should run against the real provider, not a stub","Use pending and WIP pacts to safely test provider changes"]', '["Implement can-i-deploy checks before production deployment","Use matrix builds to test against multiple provider versions","Add webhook triggers for contract change notifications","Use the testing patterns from lab-testing-patterns as a foundation for contract test structure","Add contract test coverage metrics to your quality dashboard"]') ON CONFLICT (id) DO NOTHING;