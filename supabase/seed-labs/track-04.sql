...
npm run test
npm run test:auth
`', '[{"id":"tc-1","description":"PKCE challenge verification rejects mismatched code_verifier","order":1,"required":true},{"id":"tc-2","description":"Authorization code expires after 10 minutes","order":2,"required":true},{"id":"tc-3","description":"Refresh token rotation invalidates old token family","order":3,"required":true},{"id":"tc-4","description":"State parameter prevents CSRF in authorization flow","order":4,"required":true}]', 1, '{"prerequisiteLabId": null, "stage": "Foundation", "estimatedHours": 6, "learningObjective": "Implement OAuth2 with PKCE and secure token lifecycle management", "buildsToward": "Enterprise Auth Service"}', '["Always use S256 for PKCE, never plain","State parameter must be cryptographically random, not sequential","Refresh token rotation is critical - leaked tokens become unusable"]', '["OAuth2 is not authentication - it is authorization. OpenID Connect adds auth","Token storage must encrypt at rest - never store plaintext secrets","Always validate the `aud` claim to prevent token confusion attacks"]', '["Implement token introspection endpoint for resource servers","Use DPoP (Demonstrating Proof of Possession) for sender-constrained tokens","Build a token revocation endpoint for logout flows"]'),
