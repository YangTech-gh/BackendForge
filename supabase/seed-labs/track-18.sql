...
`bash
npm run test:security
`', '[{"id":"tc-1","description":"Security headers present on all responses","order":1,"required":true},{"id":"tc-2","description":"CORS blocks requests from unauthorized origins","order":2,"required":true},{"id":"tc-3","description":"Rate limiter returns 429 with retry-after header","order":3,"required":true}]', 2, '{"prerequisiteLabId": "lab-owasp-defense", "stage": "Building", "estimatedHours": 5, "learningObjective": "Implement security headers and multi-tier rate limiting", "buildsToward": "Enterprise Security Hardening"}', '["HSTS requires initial HTTP to HTTPS redirect - plan the rollout","CORS preflight caches for max-age - consider this for changes","Sliding window rate limiting is more accurate than fixed window"]', '["Security headers are cheap - enable them all","CORS is not authentication - still validate auth tokens","Rate limiting should degrade gracefully under Redis failure"]', '["Use helmet (Node) or equivalent for automatic security headers","Implement adaptive rate limiting based on endpoint sensitivity","Add abuse detection for credential stuffing patterns"]')
