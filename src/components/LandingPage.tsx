    ...
    <main id="main-content" className="relative z-10 mx-auto flex min-h-screen w-full max-w-7xl flex-col px-5 py-6 sm:px-8 lg:px-10">
      <header className="flex items-center justify-between py-3">
        <div className="flex items-center gap-2.5 font-mono text-lg font-extrabold tracking-tight text-white">
          <span className="flex h-9 w-9 items-center justify-center rounded-xl border border-red-500/30 bg-red-500/10 text-red-400"><Network className="h-4 w-4" /></span>
      ...
      <section className="grid flex-1 items-center gap-12 py-14 lg:grid-cols-[1.08fr_.92fr] lg:py-20">
        <div className="max-w-2xl animate-fade-in-up">
          <p className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-3 py-1.5 font-mono text-xs font-semibold text-emerald-300">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" /> Practice-first backend engineering
          </p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <button onClick={onStart} className="btn-primary flex items-center gap-2 px-6 py-3.5 text-sm">Start learning free <ArrowRight className="h-4 w-4" /></button>
          ...
          <ul className="mt-8 grid gap-3 text-sm text-zinc-300 sm:grid-cols-2">
            {outcomes.map((outcome) => <li key={outcome} className="flex items-start gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-400" />{outcome}</li>)}
          </ul>
        </div>
          ...
          {testimonials.map(({ name, role, quote }) => (
            <article key={name} className="card-surface flex flex-col p-6">
              ...
              <div className="mt-5 flex items-center gap-3 border-t border-zinc-800/70 pt-4">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-red-500/10 font-mono text-sm font-bold text-red-400">{name.split(' ').map(n => n[0]).join('')}</div>
                <div>
      ...
      <section id="pricing-cta" className="border-t border-zinc-800/70 py-14">
        <div className="card-glow relative mx-auto max-w-2xl overflow-hidden rounded-2xl border border-zinc-800/70 p-8 text-center sm:p-10">
          ...
          <div className="relative">
            ...
            <button onClick={onStart} className="btn-primary mx-auto mt-6 flex items-center gap-2 px-8 py-3.5 text-sm">Get Started Free <ArrowRight className="h-4 w-4" /></button>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-4 text-xs text-zinc-500">
              <span className="flex items-center gap-1.5"><ShieldCheck className="h-4 w-4 text-emerald-400" /> 30-day money-back guarantee</span>
              <span className="flex items-center gap-1.5"><Zap className="h-4 w-4 text-amber-400" /> Instant access</span>
            </div>
          </div>
        </div>
      </section>
